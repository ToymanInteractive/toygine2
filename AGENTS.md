...
* **Frame loop and time:** Fixed-timestep simulation driven by an accumulator; presentation interpolates between the last two states. Keep simulation time, wall-clock time, and time scale distinct — never derive gameplay from a variable delta or frame count.
...
* **Job system:** Pure tasks over disjoint ranges with dependencies as a graph — no per-job allocation, no blocking waits inside a job, no shared mutable state without an ordered atomic or a phase barrier (see Concurrency primitives).
...
* **Asset pipeline:** Bake offline into a target-ready binary form; the runtime loads and uses, never parses text, converts, or validates expensively. Each format carries magic and version, and unknown versions are rejected with a clear error.
...
* **Audio:** The mixer callback is real-time — no locks, allocation, I/O, or logging. Use lock-free SPSC queues and preallocated voice pools; streaming decode runs on a worker.
