...
* **One contract, one case:** Never two cases for one behavior — if both assert the same contract, keep one. A copy-paste variant becomes a parameterized case or a small helper, and invariants of one contract belong to a single case.
* **Compile-time coverage:** Type traits, `constexpr` constructors and operators, and compile-time invariants are asserted with `static_assert`, always in the two-argument form (see **Assertions** under **Code Quality**). A runtime case covers only what constant evaluation cannot reach — behavior that depends on runtime state.
* **Unit level:** Value types, containers, math, allocators, handles, codecs — everything reachable without a running engine, where `static_assert` does most of the work (see Compile-time coverage above).
...
* **A flaky test is a defect:** Fix or delete it — never retry, skip, or quarantine. Intermittence means the test reads unspecified state or the system is non-deterministic; both are worse bugs than the test. Seed anything random explicitly — an unseeded generator leaves a failure no one can reproduce (see Determinism).
