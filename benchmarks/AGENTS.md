...
* **What a benchmark answers:** Measures a system against its declared per-frame budget (see Budgets and profiling); not a correctness test, never gates a merge. CI runs `<library>-benchmarks-run` on the desktop jobs and turns its CSV lines into a Bencher report, and a result is a number a human reads.
...
* **One binary:** every `*.benchmark.cpp` links into `<library>-benchmarks` beside the runner in `benchmarks/runner/`. A `file(GLOB_RECURSE ... CONFIGURE_DEPENDS)` in `benchmarks/CMakeLists.txt` picks up a new file, so adding one costs no CMake; `BENCHMARK_CASE` registers the cases inside it. `<library>-benchmarks-run` builds and runs the binary.
...
* **Machine-readable report:** `--csv` prints one `#csv`-prefixed line per table row beside the tables — `case,row,unit,ps_per_tick,epochs` once, then an `iters,ticks` pair per timed epoch, with the whole epoch in `ticks` and one picosecond per tick on a desktop. Naming a row once rather than once per epoch keeps the log an order of magnitude shorter. `--no-table` takes the stream away from nanobench, so a run gives the data alone. Medians, spreads and the Bencher report come from `tools/builder/bmf_from_csv.py` on the host.
