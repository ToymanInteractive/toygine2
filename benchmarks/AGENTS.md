...
* **Machine-readable report:** `--csv` prints one `#csv`-prefixed line per timed epoch beside the tables — `case,row,unit,epoch,iters,ticks,ps_per_tick`, three names then four integers, with the whole epoch in `ticks` and one picosecond per tick on a desktop. `--no-table` takes the stream away from nanobench, so a run gives the data alone. Medians, spreads and the Bencher report come from `tools/bmf_from_csv.py` on the host.
