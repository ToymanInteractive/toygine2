...
* **What a benchmark answers:** Measures a system against its declared per-frame budget (see Budgets and profiling); not a correctness test, never gates a merge. CI runs `benchmarks_run` on the desktop jobs and turns its CSV lines into a Bencher report, and a result is a number a human reads.
