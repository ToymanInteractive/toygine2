...
* **Unit tests:** DocTest, one file per public type at `tests/<module>/<name>.test.cpp`, linked against DocTest and the module under test, never an unrelated production module (see **Project Structure**). `TOYGINE_BUILD_TESTS` gates the dependency so consumers never pull it in (see Build-only dependencies).
